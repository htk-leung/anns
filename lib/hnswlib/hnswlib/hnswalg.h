...
class HierarchicalNSW : public AlgorithmInterface<dist_t> {
 public:
    ...
    mutable std::atomic<long> metric_distfunc_calls{0};
    ...
    // mutable std::atomic<long> metric_hops_upper{0};
    // mutable std::atomic<long> metric_hops_base{0};
    // /* -------------------- */
    ...
    HierarchicalNSW(
        ...
        dist_cache_ = new std::map<tableint, dist_t>; // EDIT
    }
    ...
    HierarchicalNSW(
        ...
        std::map<tableint, dist_t>* ext_cache = nullptr,
        bool allow_replace_deleted = false)
    ...
    // EDIT : new function to reset metrics
    void resetMetrics() {
        metric_distance_computations = 0;
        metric_hops = 0;
        metric_distfunc_calls = 0;
    ...
    searchBaseLayerST(
        ...
        if (bare_bone_search || 
            ...
            // EDIT : add base layer dist computation
            metric_distance_computations++;
            // metric_distcomp_base += 1;
        ...
        //  bool cur_node_deleted = isMarkedDeleted(current_node_id);
            ...
            // EDIT : removed condition to match searchKnn setting
            // if (collect_metrics) {
                metric_hops++;
                // metric_hops_base++;
                // metric_distance_computations+=size; // << wrong place??
                // metric_distcomp_base+=size;
            // }
    ...
    */
    ...
    searchKnn(const void *query_data, size_t k, BaseFilterFunctor* isIdAllowed = nullptr) const {
        ...
        if (bare_bone_search) {
            ...
            top_candidates = searchBaseLayerST<true, true>(
                    currObj, query_data, ef_/*std::max(ef_, k)*/, isIdAllowed);
        } else {
    ...
    // bare_bone_search means there is no check for deletions and stop condition is ignored in return of extra performance
    // EDIT : multiple graph search with dist comp budget
    template <bool bare_bone_search = true, bool collect_metrics = false>
    std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst>
    searchBaseLayerSTM(
        tableint ep_id,
    ...
    // EDIT : knnsearch function with dist comp budget as additional stop condition
    std::priority_queue<std::pair<dist_t, labeltype >>
    searchKnnM(
        int dc_budget,
        const void *query_data, 
        ...
        BaseFilterFunctor* isIdAllowed = nullptr) const 
    {
        ...
        if (bare_bone_search) {
            ...
            top_candidates = searchBaseLayerSTM<true, true>(
                    currObj, query_data, ef_/*std::max(ef_, k)*/, dc_budget, isIdAllowed);
        } else {
    ...
    inline dist_t 
    findDistWithCache(tableint nodeId, const void *data_point, char* ep_data) 
    {
        metric_distfunc_calls++;

        dist_t dtemp;
        // return if found in cache
        auto inCache = dist_cache_->find(nodeId);
        ...
        dist_cache_->insert({nodeId, d});

        return d;

        // // std::cout << "In findDistWithCache function: \n";
        // metric_hops++;
        // dist_t dtemp;
        // auto inCache = dist_cache_->find(nodeId);
        ...
        // if (inCache != dist_cache_->end()) {
        //     dtemp = inCache->second;
        //     std::cout << "dtemp = " << dtemp << std::endl;
        // } else {
        //     std::cout << "Before insert - Cache size: " << dist_cache_->size() << std::endl;
        //     dist_cache_->insert({nodeId, d});
        //     std::cout << "After insert - Cache size: " << dist_cache_->size() << std::endl;
        // }
        ...
        BaseSearchStopCondition<dist_t>* stop_condition = nullptr) 
    {
        ...
        if (bare_bone_search || 
            ...
            // EDIT : cache
            dist_t dist = findDistWithCache(ep_id, data_point, ep_data);
        ...
        while (!candidate_set.empty() && metric_distance_computations <= dc_budget) {
            ...
            for (size_t j = 1; j <= size; j++) {
                ...
                if (!(visited_array[candidate_id] == visited_array_tag)) {
                    ...
                    // EDIT : cache
                    dist_t dist = findDistWithCache(candidate_id, data_point, currObj1);
        ...
        BaseFilterFunctor* isIdAllowed = nullptr) 
    {
        ...
        tableint currObj = enterpoint_node_;
        dist_t curdist = findDistWithCache(enterpoint_node_, query_data,  getDataByInternalId(enterpoint_node_)); 
        ...
        for (int level = maxlevel_; level > 0; level--) {
            ...
            //          well not if you have many many graphs
                ...
                for (int i = 0; i < size; i++) {
                    ...
                    // EDIT : cache
                    dist_t d = findDistWithCache(cand, query_data, getDataByInternalId(cand));
