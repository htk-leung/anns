#ifdef INC
  #if INC == 1
    #warning "INC=1 defined - using initializeSearchRandSamp for construction"
  #else
    #warning "INC defined but not equal to 1"
  #endif
#else
  #warning "INC not defined - using standard initializeSearch for construction"
#endif
...
class Index {
  ...
  // EDIT : add variable to indicate number of entry nodes to find
  int _num_ent_s = 1;
  int _num_ent_c = 1;
  ...
  // Maybe that's what you want, but it's worth noting
    ...
    mutable std::atomic<long> metric_distcomp_upper{0};
    mutable std::atomic<long> metric_distcomp_base{0};
    mutable std::atomic<double> metric_latency_upper{0};
    mutable std::atomic<double> metric_latency_base{0};
    /* -------------------- */
 ...
 public:
  ...
  void add(void* data, label_t& label, int ef_construction, int num_initializations) {
    ...
    // EDIT : compile time define INC=randsamp to call custom function
    node_id_t entry_node;
    PriorityQueue entry_nodes;
    #if defined(INCM)
    // rather than 1 entry point now we need multiple to iterate over
        entry_nodes = initializeSearchRandSampMul(data, num_initializations, _num_ent_c); 
    #elif defined(INC) && INC == 1
    // random sampling only
        entry_node = initializeSearchRandSamp(data, num_initializations);
    #else
        entry_node = initializeSearch(data, num_initializations);
    #endif
    ...
    #if defined(INCM)
        // EDIT : create new array of neighbors from each entry point as priority queues
        // typedef std::priority_queue<dist_node_t, std::vector<dist_node_t>, CompareByFirst> PriorityQueue;
        PriorityQueue neighbors; // final priority queue
        std::set<std::pair<float, node_id_t>> neighborset; // set of neighbors
        std::pair<float, node_id_t> n; // each neighbor to test/insert

        // Find neighbors from each entry node 
        for (int i = 0; i < _num_ent_c; i++) {
            auto neighborstemp = beamSearch( // collect neighbors as priority queue
                                /* query = */ data, 
                                /* entry_node = */ entry_nodes.top().second,
                                /* buffer_size = */ ef_construction);

            // add to set to avoid duplication >> if can add to set then add to priority queue
            while(neighborstemp.size() > 0) { 
                n = neighborstemp.top();
                neighborstemp.pop();
                auto [it, inserted] = neighborset.insert(n);
                if(inserted)
                    neighbors.push(n);
            }
        }
...
//   void addMulEnt(void* data, label_t& label, int ef_construction, int num_initializations) {
...
//     // EDIT : rather than 1 entry point now we need multiple to iterate over
//     PriorityQueue entry_nodes;
//     entry_nodes = initializeSearchRandSampMul(data, num_initializations, _num_ent_c); 

//     node_id_t new_node_id; // generate new node id (basically equal to its index number)
//     allocateNode(data, label, new_node_id);
//     global_lock.unlock();

//     if (new_node_id == 0) { // if it's the first node in the graph there's nothing else to do
//       return;
//     }
...
//   }
  ...
  std::vector<dist_label_t> search(const void* query, const int K, int ef_search,
    ...
    // EDIT : call custom functions
    std::chrono::time_point<std::chrono::high_resolution_clock> t2;
    #if defined(INSM)
    // if defined macro INSM___ call custom function
        PriorityQueue entry_nodes = initializeSearchRandSampMul(data, num_initializations, _num_ent_s); 
    ...
    // if defined macro INS=___ call custom function
        node_id_t entry_node = initializeSearchRandSamp(query, num_initializations);
        t2 = std::chrono::high_resolution_clock::now();
        neighbors = beamSearch(/* query = */ query,
  ...
  inline void setNumEntrySearch(uint32_t num) {
    _num_ent_s = num;
  }

  inline void setNumEntryConstr(uint32_t num) {
    _num_ent_c = num;
  }
  ...
  inline uint64_t metricHops() const { return _metric_hops.load(); }
  inline uint64_t distanceComputationsUpper() const { return metric_distcomp_upper.load(); }
  inline uint64_t distanceComputationsBase() const { return metric_distcomp_base.load(); }
  inline double latencyUpper() const { return metric_latency_upper.load(); }
  inline double latencyBase() const { return metric_latency_base.load(); }
  ...
  void resetStats() {
    ...
    _metric_hops = 0;
    /* NEW METRIC VARIABLES */
    metric_distcomp_upper = 0;
    metric_distcomp_base = 0;
    metric_latency_upper = 0;
    metric_latency_base = 0;
  }
 ...
 private:
  ...
  PriorityQueue beamSearch(const void* query, const node_id_t entry_node, 
    ...
    if (_collect_stats) {
        ...
        metric_distcomp_base.fetch_add(1);
    }
  ...
  void selectNeighbors(PriorityQueue& neighbors, int M) {
    ...
    // add neighbors to candidates priority queue with -distance so closest is at the top
    ...
    while (candidates.size() > 0) { // for each candidate, start from closest
      ...
      for (const auto& [_, second_pair_node_id] : saved_candidates) { // go through current selection
        float cur_dist = _distance->distance(/* x = */ getNodeData(second_pair_node_id),
                                       /* y = */ getNodeData(current_node_id)); // calculate distance between neighbor1 and neighborX

        if (cur_dist < distance_to_query) { // if distance between neighbor1 and neighborX is smaller than that to Q
          should_keep_candidate = false;    // don't keep 
          break;
        }
      }
  ...
  inline node_id_t initializeSearch(const void* query, int num_initializations) {
    ...
    if (_collect_stats) {
      ...
      metric_distcomp_upper.fetch_add(num_initializations);
    }
  ...
  // EDIT : in this version of initializeSearch
  // a random set of num_init nodes are sampled instead of using stepsize
  inline node_id_t initializeSearchRandSamp(const void* query, int num_initializations) {
    // select entry_node from a set of random entry point options
    if (num_initializations <= 0) {
      throw std::invalid_argument("num_initializations must be greater than 0.");
    }
    ...
    // rng generates pseudorandom values by using it with ()
    std::random_device rd;
    std::mt19937 rng(rd());

    // increment dist computation count
    if (_collect_stats) {
      _distance_computations.fetch_add(num_initializations);
      metric_distcomp_upper.fetch_add(num_initializations);
    }
    ...
    // save value to candidates and ensure uniqueness by using unordered_set data structure
    std::unordered_set<node_id_t> candidates;
    node_id_t temp;
    if(_cur_num_nodes < num_initializations)
      for (int i = 0; i < _cur_num_nodes; i++)
        candidates.insert(i);
    else
      while(candidates.size() < num_initializations) {
        temp = rng() % (!_cur_num_nodes ? 1 : _cur_num_nodes);
        candidates.insert(temp);
      }

    // find minimum in array and set as entry_node
    for (auto it = candidates.begin(); it != candidates.end(); it++) {
      float dist = _distance->distance(/* x = */ query, /* y = */ getNodeData(*it),
                                       /* asymmetric = */ true);
      if (dist < min_dist) {
        min_dist = dist;
        entry_node = *it;
  ...
  // EDIT : in this version of initializeSearch
  // a random set of num_init nodes are sampled instead of using stepsize
  inline PriorityQueue initializeSearchRandSampMul(const void* query, int num_initializations, int num_ent) {
    // select entry_node from a set of random entry point options
    if (num_initializations <= 0) {
      throw std::invalid_argument("num_initializations must be greater than 0.");
    }
    ...
    std::unordered_set<node_id_t> candidates;
    node_id_t temp;
    if(_cur_num_nodes < num_initializations) // not enough nodes to start with, just add all of them
      for (int i = 0; i < _cur_num_nodes; i++)
        candidates.insert(i);
    else
      while(candidates.size() < num_initializations) { // random sampling
        temp = rng() % (!_cur_num_nodes ? 1 : _cur_num_nodes);
        candidates.insert(temp);
      }

    // EDIT : new method
    // candidates saved into std::unordered_set<node_id_t> candidates; 
    PriorityQueue cpq;
    for (auto it = candidates.begin(); it != candidates.end(); it++) {  // go through samples 
        float dist = _distance->distance(/*x = */ query,                // calculate distance
                                            /* y = */ getNodeData(*it),
                                            /* asymmetric = */ true);
        cpq.emplace(dist, *it);                                         // save to priority queue
    }
    // call selectneighbor to get neighbors : void selectNeighbors(PriorityQueue& neighbors, int M)
    selectNeighbors(cpq, num_ent);
    
    return cpq; // return MULTIPLE entry nodes
  }
