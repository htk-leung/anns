...
#define resultfile "../../logresult/20251002-flatnav-gen.txt"
...
float run_knn_search(flatnav::Index<dist_t, int>* index, float *qs, int* gt, int efs, int k, int gtnum, int qnum, int dim) {
  ...
  for (int i = 0; i < qnum; i++) {            // for each query item
    ...
    std::vector<std::pair<float, int>> result = index->search(q, k, efs); // list of NN
...
float recall_flatnav(int testnum, int dim, float *data, float *qs, int *gt, int datasize, int qnum, int gtnum,
  int m, int efc, int efs, int k, std::ofstream& result) {
    ...
    float recall = run_knn_search(index, qs, gt, efs, k, gtnum, qnum, dim);
...
int main(int argc, char **argv) {
    ...
    for (int efs = 100; efs <= 400; efs*=2) // k
    {
        for (int m = 32; m <=128; m*=2)
        {
            ...
            {
              recall_flatnav(testnum++, DIM, data, qs, gt, DATASIZE, QNUM, GTNUM, m, efc, efs, GTNUM, result);
            }
        }
    }
