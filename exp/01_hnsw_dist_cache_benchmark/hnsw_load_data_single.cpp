...
/* LOG FILE */
    ...
    #define resultfile "../../logresult/20251002-hnsw-dist-cache-TEST.txt"
#else
    // #define logfile "../../logresult/20251002-hnsw-dist-nocache-gen.log"
    #define resultfile "../../logresult/20251002-hnsw-dist-nocache-TEST.txt"
#endif
...
float recall_data_k_cache(int testnum, int dim, int max_elements, int k, int numq, int m, int ef_construction, int efs, 
    ...
    std::cout << "kNN Test " << testnum << ":\nRecall: " << recall << std::endl;
        ...
        result << dim << "," << ef_construction << "," << m << "," << efs << "," << k << "," << recall << "," << duration << std::endl;     //"d,efc,m,efs,k,recall"
    #endif
...
int main() {
    ...
    try {
        ...
        qs = bin_to_arr<float>(qsfile, QSFILENAME, QSSIZE);
        gt = bin_to_arr<int>(gtfile, GTFILENAME, GTSIZE);
    ...
    //         {
    //             recall_data_k_cache(testnum++, d, DATASIZE, QSSIZE, GTSIZE, m, efc, efs, result, fstdistfunc_cache_, data, qs, gt);
    //             dist_cache_.clear();
    //             fstdistfunc_counter_ = 0;
    //         }
    //     }
    // }
    int m = 32, efc = 50, efs = 100, d = 128;
    recall_data_k_cache(1, d, DATASIZE, QSSIZE, GTSIZE, m, efc, efs, result, fstdistfunc_cache_, data, qs, gt);
    dist_cache_.clear();
